<script lang="ts">
	...
	onMount(async () => {
		await Promise.all([loadEmployeeTabs(), loadAgencyOSChats()]);
	});
	...
	function currentSubAccountId(): string | null {
		const orgWithSettings = $activeOrg as { settings?: Record<string, unknown> } | null;
		return (
			(orgWithSettings?.settings?.sub_account_id as string | undefined) ??
			(orgWithSettings?.settings?.subaccount_id as string | undefined) ??
			(orgWithSettings?.settings?.workpipe_sub_account_id as string | undefined) ??
			(orgWithSettings?.settings?.workpipe_subaccount_id as string | undefined) ??
			null
		);
	}
...
>
	...
	<main class="flex-1 flex flex-col relative z-10 min-w-0">
		...
		<div
			class="absolute top-0 left-0 right-0 z-20 flex items-center justify-center py-3 px-3 sm:px-6 bg-gradient-to-b from-[rgba(20,20,25,0.9)] to-transparent h-20 sm:h-24 pointer-events-none"
		>
			<!-- Mobile hamburger -->
			<button
